...
router.get('/:itemId/details', async (req, res) => {
    ...
    try {
        ...
        const owner = await userService.getUserById(item.owner);
        const users = await userService.getAllUsersByIds(item.usersList);
        const usersList = users.join(', ');

        res.render('items/details', { item, isOwner, isLiked,owner, users, usersList, title: 'Details Page' });
    } catch (error) {
